...
class DDQNAgent():
    ...
    def _build_model(self):
        input_layer = Input(shape=(self.obvs_space,))
        dense_1 = Dense(32, activation='relu', kernel_initializer='he_normal')(input_layer)
        dense_2 = Dense(32, activation='relu', kernel_initializer='he_normal')(dense_1)

        advantages = Dense(self.action_space, activation='linear', kernel_initializer='he_normal')(dense_2)
        mean = Lambda(lambda x: K.mean(x, axis = 1, keepdims=True))(advantages)
        adv = subtract([advantages, mean])    
        value = Dense(1, activation='linear')(dense_2)
        policy = add([adv, value])
